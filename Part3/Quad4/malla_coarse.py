...
for e in Quadrangles:
	...
	if e in range(1,8) or range(116,141):
		properties["t"] = 5e-3
	else:
		properties["t"] = 4e-3
...
properties_load["tx"] = 1e3/(properties_load["t"]*4e-2) #(kN/m)/H -> kN/()
...
write_node_data("post/coarse/ux_coarse.msh", nodes, uv[:,0], "Despl. X")
write_node_data("post/coarse/uy_coarse.msh", nodes, uv[:,1], "Despl. Y")
write_node_data_2("post/coarse/desplazamientos_coarse.msh", nodes, uv[:,0], uv[:,1], "Despl")
...
write_element_data("post/coarse/sigma_x_coarse.msh", elementos, σxx, "Sigma_x")
write_element_data("post/coarse/new_sigma_x_coarse.msh", elementos, new_σxx, "New_Sigma_x")